...
func UploadFile(c *gin.Context) {
	...
	if err != nil {
		global.SdLog.Error("接收文件失败!", zap.Any("err", err))
		response.FailWithMessage("接收文件失败", c)
		return
	}
	err, file = service.UploadFile(header, noSave) // 文件上传后拿到文件路径
	if err != nil {
		global.SdLog.Error("修改数据库链接失败!", zap.Any("err", err))
		response.FailWithMessage("修改数据库链接失败", c)
		return
	}
	response.OkWithDetailed(response.FileResponse{File: file}, "上传成功", c)
}
...
// @Security ApiKeyAuth
// @Produce  application/json
// @Param data body model.FileUploadAndDownload true "传入文件里面id即可"
