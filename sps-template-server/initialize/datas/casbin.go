...
func InitCasbinModel(db *gorm.DB) {
	if err := db.Transaction(func(tx *gorm.DB) error {
		if tx.Where("p_type = ? AND v0 IN ?", "p", []string{"888", "8881", "9528"}).Find(&[]model.CasbinRule{}).RowsAffected == 142 {
			global.SdLog.Error("casbin_rule表的初始数据已存在!")
			return nil
		}
		...
		global.SdLog.Warn("[Mysql]--> casbin_rule 表的初始数据失败,err: %v\n", zap.Any("err", err))
		os.Exit(0)
	}
}
