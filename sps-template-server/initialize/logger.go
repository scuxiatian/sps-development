...
func (g *GormLogger) Printf(message string, data ...interface{}) {
	if global.SdConfig.Mysql.LogZap == "Info" && !global.SdConfig.Mysql.LogMode {
		switch len(data) {
		case 0:
			global.SdLog.Info(message)
		case 1:
			global.SdLog.Info("gorm", zap.Any("src", data[0]))
		case 2:
			global.SdLog.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
			global.SdLog.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
		case 4:
			global.SdLog.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]), zap.Any("sql", data[3]))
		}
		return
	}
	switch len(data) {
